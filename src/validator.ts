...
function err(code: string, message: string, lineNumber?: number): Diagnostic {
  return { severity: "error", code, message, ...(lineNumber !== undefined ? { lineNumber } : {}) };
}

function rec(code: string, message: string, lineNumber?: number): Diagnostic {
  return {
...
export function collectErrors(
  ...
  const contacts = fields.filter((f): f is ContactField => f.type === "contact");
  const expiresFields = fields.filter((f): f is ExpiresField => f.type === "expires");
  const preferredLangsFields = fields.filter(
    (f): f is PreferredLanguagesField => f.type === "preferred-languages",
  );
  ...
  if (expiresFields.length > 1) {
    const second = expiresFields[1] as ExpiresField;
    diagnostics.push(
      err(
        "MULTIPLE_EXPIRES",
  ...
  if (preferredLangsFields.length > 1) {
    const second = preferredLangsFields[1] as PreferredLanguagesField;
    diagnostics.push(
      err(
  ...
  for (const field of fields) {
    if (field.type === "acknowledgments") {
      ...
      const enc = field as EncryptionField;
      ...
      const pl = field as PreferredLanguagesField;
...
export function collectRecommendations(
  ...
  const encryption = fields.filter((f): f is EncryptionField => f.type === "encryption");
  ...
  for (const field of fields) {
    const value =
      field.type === "extension"
        ? (field as ExtensionField).value
        : (field as Exclude<ParsedField, ExtensionField>).value;
    if (value.length > 2048) {
      const name = field.type === "extension" ? (field as ExtensionField).name : field.type;
      diagnostics.push(
        rec(
          "FIELD_TOO_LONG",
