...
interface PgpParseResult {
  pgp: PgpInfo;
  body: string;
}

function stripPgpArmor(input: string): PgpParseResult {
  ...
  let state: "header" | "body" | "sig" | "done" = "header";
...
function buildContactField(raw: RawField): ContactField {
  return {
    type: "contact",
    value: raw.value,
...
function buildExpiresField(raw: RawField): ExpiresField {
  return {
    type: "expires",
    value: raw.value,
...
function buildEncryptionField(raw: RawField): EncryptionField {
  return {
    type: "encryption",
    value: raw.value,
    scheme: getScheme(raw.value),
...
function buildPreferredLanguagesField(raw: RawField): PreferredLanguagesField {
...
function buildSimpleField(
  raw: RawField,
  type: "acknowledgments" | "canonical" | "hiring" | "policy",
): AcknowledgmentsField | CanonicalField | HiringField | PolicyField {
  return { type, value: raw.value, lineNumber: raw.lineNumber } as
    | AcknowledgmentsField
    | CanonicalField
    | HiringField
    | PolicyField;
...
export function parse(input: string, options: ParseOptions = {}): ParseResult {
  ...
  const { pgp, body } = options.skipPgpStripping
    ? { pgp: { signed: false, hashAlgorithms: [], signatureBlock: "" } as PgpInfo, body: input }
    : stripPgpArmor(input);
  ...
  for (let i = 0; i < rawLines.length; i++) {
    ...
    const raw: RawField = { name: rawName, value: rawValue, lineNumber };
  ...
  return {
    contacts: fields.filter((f): f is ContactField => f.type === "contact"),
    expires: fields.find((f): f is ExpiresField => f.type === "expires") ?? null,
    acknowledgments: fields.filter((f): f is AcknowledgmentsField => f.type === "acknowledgments"),
    canonical: fields.filter((f): f is CanonicalField => f.type === "canonical"),
    encryption: fields.filter((f): f is EncryptionField => f.type === "encryption"),
    hiring: fields.filter((f): f is HiringField => f.type === "hiring"),
    policy: fields.filter((f): f is PolicyField => f.type === "policy"),
    preferredLanguages:
      fields.find((f): f is PreferredLanguagesField => f.type === "preferred-languages") ?? null,
    extensions: fields.filter((f): f is ExtensionField => f.type === "extension"),
    fields,
    errors,
    recommendations,
    pgp,
