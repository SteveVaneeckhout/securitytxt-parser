...
function securityTxtUrl(siteUrl: string): URL {
...
async function fetchWithRedirects(initialUrl: URL, opts: ResolvedOptions): Promise<FetchOutcome> {
    ...
    try {
      ...
      throw new FetchError(
        isAbort
          ...
          : `Failed to fetch ${currentUrl.href}: ${(err as Error).message}`,
        currentUrl.href,
        null,
      );
    }
...
export async function fetchSecurityTxt(
  ...
  const parseOptions: { now?: Date; skipPgpStripping?: boolean } = {};
